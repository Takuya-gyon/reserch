...
# データが保存されているフォルダのパス
data_folder = ".//data//行列時系列"

#出力先フォルダ
output_folder = ".//output//行の時系列グラフ//"
...
def create_question_pdf2(data_folder, questions, pointN, ifCorrect=False):
    pdf_filename = output_folder + f"output2_{'正解者' if ifCorrect else '不正解者'}.pdf"
    with PdfPages(pdf_filename) as pdf:
        for question_label in questions:
            ...
            for name in names:
                file_path = os.path.join(data_folder, f"{name}-{question}.csv")
